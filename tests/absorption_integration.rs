...
use neurax_parser::{parse_model_config, AbsorbedModel, IrInjector};
use neurax_ir::{ArchitectureIRInput, MemoryPassConfig, HardwarePassConfig, CostPassConfig};
...
fn test_llama7b_absorption() {
    ...
    assert!(total_params > 6_000_000_000, "Expected > 6B params, got {}", total_params);
    assert!(total_params < 8_000_000_000, "Expected < 8B params, got {}", total_params);
...
#[test]
fn test_confidence_score_degradation() {
    // Test with minimal JSON - should have lower confidence
    let minimal_json = r#"
    {
        "schema_version": "1.0",
        "model": {
    ...
    let config = parse_model_config(minimal_json).unwrap();
    let absorbed = AbsorbedModel::absorb(config);
    
    // Should have missing fields and lower confidence
    assert!(!absorbed.resolution_context.missing_fields.is_empty(), 
            "Should have missing fields for minimal JSON");
}
